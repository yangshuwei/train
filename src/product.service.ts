...
export class ProductService{
  ...
  async submitOrder(jsondata) {
    ...
    const data = {
      ...
      password: process.env.PAYPASSWORD,
      codTimeType: 3,
      paymentType: 4,
      areaCode: '',
      overseas: 0,
    ...
    const headers = {
      'User-Agent': this.loginService.ua,
      Host: 'marathon.jd.com',
      Referer: `https://marathon.jd.com/seckill/seckill.action?skuId=${productId}&num=${process.env.BYNUM}&rid=${Date.now()}`,
      cookie: this.loginService.cookieToHeader(),
    };
